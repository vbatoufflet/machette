...
class UndoStack:
    ...
    def _delete_range(self, source=None, start=None, end=None):
        ...
        Handle text range deletion
            void _delete_range(event source: gtk.Object,
                               start iter: gtk.TextIter,
                               end iter: gtk.TextIter)
        """
    ...
    def _end_action(self, source=None, event=None):
        ...
        if len(self.undostack) > 0 \
          and self.undostack[-1][0] == self.action \
          and self.start == self.undostack[-1][1] + len(self.undostack[-1][2]):
    ...
    def _handle_keybinding(self, source=None, event=None):
        ...
        Register an user action
            void _handle_keybinding(event source: gtk.Object,
                                    event: gtk.gdk.Event)
        """
    ...
    def _insert_text(self, source=None, start=None, text=None, length=None):
        ...
        Handle text insertions
            void _insert_text(event source: gtk.Object,
                              start iter: gtk.TextIter, inserted text: str,
                              text length: int)
        """
    ...
    def _redo(self, source=None, event=None):
        ...
        if item[0] == 'delete':
            self.buffer.delete(self.buffer.get_iter_at_offset(item[1]),
                               self.buffer.get_iter_at_offset(item[1] +
                                                              len(item[2])))
        elif item[0] == 'insert':
            self.buffer.insert(self.buffer.get_iter_at_offset(item[1]),
                               item[2])
...
gobject.signal_new('redo-event', gtk.TextBuffer, gobject.SIGNAL_RUN_FIRST,
                   gobject.TYPE_NONE, ())
gobject.signal_new('undo-event', gtk.TextBuffer, gobject.SIGNAL_RUN_FIRST,
                   gobject.TYPE_NONE, ())
